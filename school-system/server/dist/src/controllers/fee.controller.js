...
export const feeController = {
    ...
    paymentHistory: asyncHandler(async (req, res) => {
        requireAdmin(req);
        return sendSuccess(res, await feeService.paymentHistory(Number(req.params.studentId)), "Payment history");
    }),
    outstanding: asyncHandler(async (req, res) => {
        requireAdmin(req);
        return sendSuccess(res, await feeService.outstandingFees(Number(req.params.studentId)), "Outstanding fees");
    }),
    monthlyReport: asyncHandler(async (req, res) => {
        requireAdmin(req);
        return sendSuccess(res, await feeService.monthlyReport(Number(req.query.month), Number(req.query.year)), "Monthly fee report");
    }),
};
